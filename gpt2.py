...
class GPT2Attention(nn.Module):
    def __init__(self, config, is_cross_attention=False, layer_idx=None):
        ...
        self.register_buffer(
            "causal_bias",
            torch.tril(torch.ones((20000, 20000), dtype=torch.uint8)).view(
                1, 1, 20000, 20000
            ),
        )
        '''
    ...
    def _attn(self, query, key, value, attention_mask=None, head_mask=None):
        ...
        if not self.is_cross_attention:
            ...
            if key_length > self.bias.size(-1):
                causal_mask = self.bias[:, :, :query_length, :query_length].to(torch.bool)

                attn_weights_qk = torch.where(causal_mask, attn_weights[:, :, :, key_length - query_length: ], self.masked_bias.to(attn_weights.dtype))
                attn_weights = torch.cat([attn_weights[:, :, :, :key_length - query_length], attn_weights_qk], -1)
            else:
...
class GPT2LMHeadModelManyShot(GPT2PreTrainedModel):
